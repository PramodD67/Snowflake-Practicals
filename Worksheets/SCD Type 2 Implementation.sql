...
-- 1️⃣ DELETE Operation → Close record
WHEN MATCHED 
...
-- 2️⃣ UPDATE Operation → Data changed → close + insert new
WHEN MATCHED 
AND S.METADATA$ACTION = 'UPDATE'
AND (
       NVL(T.SALUTATION,'') <> NVL(S.SALUTATION,'')
    OR NVL(T.FIRST_NAME,'') <> NVL(S.FIRST_NAME,'')
    OR NVL(T.LAST_NAME,'') <> NVL(S.LAST_NAME,'')
    OR NVL(T.EMAIL_ADDRESS,'') <> NVL(S.EMAIL_ADDRESS,'')
    OR NVL(T.SCHEMA_EV_COL_GENDER,'') <> NVL(S.SCHEMA_EV_COL_GENDER,'')
) THEN
    -- expire old row
    UPDATE SET 
        T.END_DATE   = CURRENT_DATE,
...
-- 3️⃣ INSERT Operation → New record
WHEN NOT MATCHED 
...
-- Summary (What you say in interview)
------------------------------------------------
-- Your code as given is NOT a valid SCD2 implementation.
-- To implement SCD2, you MUST include:**
-- Match only on business key
